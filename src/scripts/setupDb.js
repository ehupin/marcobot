...
async function setupDb(){
    ...
    let exchangeItems = await Promise.all(exchanges.map(listExchangeRelatedNodesAndEdges))
    ...
    return
    let data = JSON.stringify(allItems, null, 4);  
...
async function listExchangeRelatedNodesAndEdges(exchange){
    ...
    for (let currencyName of Object.keys(currencies)){
