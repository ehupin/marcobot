import fs from 'fs'
import {connectDb, clearDatabase, createDbNode, connectDbNodes, getExchangeMarkets, setMarketPrice} from './database.js'
...
request('https://exchangebit.info/bittrex', function(err, resp, html) {
        if (!err){
          ...
          $(`html body main.container table.table.table-striped.border.table-sm tr`).each((i,tr)=>{
            ...
            const coinLabelElem = tds.get(1)
            if (!coinLabelElem){return}
            const coinLabel = coinLabelElem.children[0].children[0].data
            
            const withdrawalFeesElem = tds.get(3)
            if (!withdrawalFeesElem){return}
            const withdrawFees = withdrawalFeesElem.children[0].data
            
            console.log( withdrawFees, coinLabel)
