...
export async function connectDbNodes (db, collectionName, sourceNodeId, destNodeId, data) {
...
export async function setMarketPrice(db, market){
    const nodeDetails = {label: market.label}
    const nodeData = {price: market.price}
    const cursor = await db.query(aql`
    FOR market IN markets FILTER market.label == ${market.label}
    UPDATE market WITH ${nodeData} IN  ${db.edgeCollection('markets')}
    return market
    `);
}

export async function getExchangeMarkets(db, exchangeName){
    const cursor = await db.query(aql`
