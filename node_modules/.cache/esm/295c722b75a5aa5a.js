    ...
    return {
        add(type, from, to){
            ...
            if (!Object.keys(edgeTree).includes(from)){
                edgeTree[type][from._id] = [to._id]
            }
            else {
                edgeTree[type][from._id].push(to._id)
            }
            // console.log('\n==================\n',type, from,to, '\n',edgeTree,'\n\n')
        },
        out(){
          _75a‍.g.console.log(JSON.stringify(edgeTree, null, 4))
        },
        build(){
            for (let type of Object.keys(edgeTree)){
                for (let from of Object.keys(edgeTree[type])){
                    for (let to of edgeTree[type][from]){
                        _75a‍.g.console.log('{{{{{{--',from,to)
