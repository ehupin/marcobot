"main";let fs;_b4b‍.w("fs",[["default",["fs"],function(v){fs=v}]]);let connectDb,clearDatabase,createDbNode;_b4b‍.w("./database.js",[["connectDb",["connectDb"],function(v){connectDb=v}],["clearDatabase",["clearDatabase"],function(v){clearDatabase=v}],["createDbNode",["createDbNode"],function(v){createDbNode=v}]]);
        ...
        for (let exchangeModuleName of Object.keys(exchangeModules)){
            ...
            for (let market of markets){
                ...
                if (!Object.keys(marketNodes).includes(marketLabel)){
                    marketNodes[marketLabel] = await createDbNode('market', {label: marketLabel})
                }
                
                if (!Object.keys(currencyNodes).includes(pair.baseCurrency)){
                    currencyNodes[pair.baseCurrency] = await createDbNode('currency', {label: pair.baseCurrency})
                }
                
                if (!Object.keys(currencyNodes).includes(pair.quoteCurrency)){
                    currencyNodes[pair.quoteCurrency] = await createDbNode('currency', {label: pair.quoteCurrency})
                }
                
                if (!Object.keys(exchangeNodes).includes(pair.exchange)){
                    exchangeNodes[pair.exchange] = await createDbNode('exchange', {label: pair.exchange})
                }
