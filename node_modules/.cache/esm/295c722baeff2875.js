...
async function setupDb(){
    ...
    fs.readdir('exchanges', async (err,moduleFiles)=>{
        ...
        for (let exchangeModuleName of Object.keys(exchangeModules)){
            ...
            // load module and get markets
            const module = exchangeModules[exchangeModuleName][exchangeModuleName]
            const markets = await module.fetchMarkets()
            
            // build objects to store nodes
            let marketNodes = {}
            let currencyNodes = {}
            let exchangeNodes = {}
            
            // build exchange node, add it to exchangeNodes object
            const exchangeNodeData = {
                                        label: exchangeModuleName,
                                        tradingFees: module.tradingFees
                                    }
            exchangeNodes[exchangeModuleName] = await createDbNode(db, 'exchanges', exchangeNodeData)
            
            // build the edge temp object
            let tempEdges = TempEdges()
            
            // loop throught markets to fill marketNodes and currencyNodes, also add edges to tempEdges object
            for (let market of markets){
                ...
                _aef‍.g.console.log(marketNodes)
                _aef‍.g.console.log(currencyNodes)
                _aef‍.g.console.log(exchangeNodes)
                ...
                tempEdges.add('exchangeHasCurrency', 
                            ...
                            baseCurrencyNode, 
                            {
                                withdrawalFees: exchangeModules[market.exchange][market.exchange].withdrawalFees[market.baseCurrency]
