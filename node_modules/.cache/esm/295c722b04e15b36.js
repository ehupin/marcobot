...
async function setupDb(){
    ...
    fs.readdir('exchanges', async (err,moduleFiles)=>{
        ...
        for (let exchangeModuleName of Object.keys(exchangeModules)){
            ...
            let tempEdgeTree = TempEdgesTree()
            ...
            for (let market of markets){
                ...
                if (!Object.keys(currencyNodes).includes(market.baseCurrency)){
                    currencyNodes[market.baseCurrency] = await createDbNode(db, 'currency', {label: market.baseCurrency})
                }
                ...
                _04e‍.g.console.log(marketNode,'\n', exchangeNode, '\n',baseCurrencyNode, '\n',quoteCurrencyNode)
                
                tempEdgeTree.add('marketHasExchange', marketNode, exchangeNode)
                tempEdgeTree.add('marketHasBaseCurrency', marketNode, baseCurrencyNode)
                tempEdgeTree.add('marketHasQuoteCurrency', marketNode, quoteCurrencyNode)
                tempEdgeTree.add('exchangeHasCurrency', exchangeNode, baseCurrencyNode)
                tempEdgeTree.add('exchangeHasCurrency', exchangeNode, quoteCurrencyNode)
                return
            }
            // tempEdgeTree.out()
            // edgeTree.build()
