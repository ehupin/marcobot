import fs from 'fs'
import {connectDb, clearDatabase, createDbNode, connectDbNodes} from './database.js'
...
async function setupDb(){
    ...
    for (let exchangeName of Object.keys(exchangeModules)){
        const exchangeNodeData = {
                                    label: exchangeName,
                                    tradingFees: exchangeModules[exchangeName].tradingFees
                                }
        exchangeNodes[exchangeName] = await createDbNode(db, 'exchanges', exchangeNodeData)
    }
    ...
    for (let exchangeModuleName of Object.keys(exchangeModules)){
        ...
        for (let market of markets){
            ...
            if (!Object.keys(marketNodes).includes(marketLabel)){
                marketNodes[marketLabel] = await createDbNode(db, 'markets', {label: marketLabel, price: null, urlLabel:market.urlLabel})
            }
